...
// ============================================================
// ALL 32 ABILITY ICONS — illustrative, function-representative
// ============================================================
const icons = {

  // ---- TIER 1 FREE ----

  // Earthquake: rows shifting left/right with crack
  ...
  // Screen Shake: monitor vibrating with motion lines
  ...
  // Ink Splash: actual ink splatter blobs
  ...
  // Mini Blocks: big block shrinking to small domino
  ...
  // Fill Holes: blocks dropping into gaps
  ...
  // Clear Rows: 4 rows being wiped with sweep line
  clear_rows: (c = "#fff", s = 32) => (
    <svg width={s} height={s} viewBox="0 0 32 32" fill="none">
      ...
      {/* Sweep / wipe effect - horizontal line */}
  ...
  // Random Spawner: blocks appearing with sparkles
  ...
  // Garbage Rain: blocks falling down like rain
  ...
  // Speed Up: piece falling fast with speed lines
  speed_up: (c = "#fff", s = 32) => (
    <svg width={s} height={s} viewBox="0 0 32 32" fill="none">
      {/* T-piece falling */}
  ...
  // Cascade Multiplier: stars with x2
  ...
  // Shield: hexagonal shield with check
  shield: (c = "#fff", s = 32) => (
    <svg width={s} height={s} viewBox="0 0 32 32" fill="none">
  ...
  // Reverse Controls: mirrored arrows crossing
  reverse_controls: (c = "#fff", s = 32) => (
    <svg width={s} height={s} viewBox="0 0 32 32" fill="none">
      {/* Left arrow pointing right (reversed!) */}
      ...
      {/* Right arrow pointing left (reversed!) */}
  ...
  // Mirage: piece preview with question marks
  mirage: (c = "#fff", s = 32) => (
    <svg width={s} height={s} viewBox="0 0 32 32" fill="none">
      {/* Preview box */}
  ...
  // Gold Digger: pickaxe removing cells
  gold_digger: (c = "#fff", s = 32) => (
    <svg width={s} height={s} viewBox="0 0 32 32" fill="none">
      {/* Grid cells - some being removed */}
  ...
  // Column Swap: two columns with swap arrows
  ...
  // Gravity Well: blocks collapsing downward
  ...
  // Overcharge: lightning bolt with discount
  ...
  // Clone: mirror/copy symbol
  clone: (c = "#fff", s = 32) => (
    <svg width={s} height={s} viewBox="0 0 32 32" fill="none">
      {/* Original ability (faded) */}
  ...
  // Rotation Lock: piece with lock symbol, no rotation
  ...
  // Blind Spot: board with bottom hidden in darkness
  blind_spot: (c = "#fff", s = 32) => (
    <svg width={s} height={s} viewBox="0 0 32 32" fill="none">
      ...
      {/* Black zone covering bottom 5 rows */}
  ...
  // Cross Firebomb: cross pattern explosion
  ...
  // Weird Shapes: bizarre 3x3 hollow frame piece
  ...
  // Narrow Escape: board narrowing with arrows
  ...
  // Reflect: mirror bouncing ability back
  reflect: (c = "#fff", s = 32) => (
    <svg width={s} height={s} viewBox="0 0 32 32" fill="none">
      {/* Mirror surface */}
      ...
      {/* Shield outline on mirror */}
      <path d="M15.5 20L15.5 26" stroke={c} strokeWidth="0.8" strokeLinecap="round" opacity="0.15" />
    </svg>
  ),

  // Magnet: piece snapping to optimal spot
  magnet: (c = "#fff", s = 32) => (
    <svg width={s} height={s} viewBox="0 0 32 32" fill="none">
      {/* Board with gap */}
  ...
  // Quicksand: bottom rows sinking with sand effect
  ...
  // Shapeshifter: piece morphing between shapes
  shapeshifter: (c = "#fff", s = 32) => (
    <svg width={s} height={s} viewBox="0 0 32 32" fill="none">
      {/* Shape 1: T-piece (faded) */}
  ...
  // Shrink Ceiling: ceiling pressing down
  ...
  // Wide Load: board expanding outward
  ...
  // Tilt: piece drifting diagonally
  tilt: (c = "#fff", s = 32) => (
    <svg width={s} height={s} viewBox="0 0 32 32" fill="none">
      {/* Board with tilt line */}
      <rect x="6" y="3" width="20" height="26" rx="1.5" stroke={c} strokeWidth="1" opacity="0.2" />
      {/* Diagonal gravity arrow */}
  ...
  // Flip Board: board rotating 180°
  flip_board: (c = "#fff", s = 32) => (
    <svg width={s} height={s} viewBox="0 0 32 32" fill="none">
      {/* Board - before (faded, upright) */}
      ...
      {/* Board - after (solid, flipped/inverted) */}
...
function SkillIcon({ icon, color = "#fff", size = 32, glow = false }) {
  ...
  return (
    ...
    }}>{render(color, size)}</div>
  );
}

const ALL_ABILITIES = [
  { id: "earthquake", name: "Earthquake", cost: 40, tier: 1, cat: "debuff_board", color: "#00f0f0" },
  { id: "screen_shake", name: "Screen Shake", cost: 40, tier: 1, cat: "debuff_visual", color: "#ff6090" },
  { id: "ink_splash", name: "Ink Splash", cost: 35, tier: 1, cat: "debuff_visual", color: "#b040f0" },
  { id: "mini_blocks", name: "Mini Blocks", cost: 35, tier: 1, cat: "buff_pieces", color: "#ff40a0" },
  { id: "fill_holes", name: "Fill Holes", cost: 75, tier: 1, cat: "buff_board", color: "#40ff90" },
  { id: "clear_rows", name: "Clear Rows", cost: 100, tier: 1, cat: "buff_board", color: "#f0e000" },
  { id: "random_spawner", name: "Random Spawner", cost: 45, tier: 2, cat: "debuff_board", color: "#f0a020" },
  { id: "garbage_rain", name: "Garbage Rain", cost: 55, tier: 2, cat: "debuff_board", color: "#ff6040" },
  { id: "speed_up", name: "Speed Up", cost: 60, tier: 2, cat: "debuff_controls", color: "#ff3c50" },
  { id: "circle_bomb", name: "Circle Bomb", cost: 65, tier: 2, cat: "buff_board", color: "#ff6040" },
  { id: "cascade_multiplier", name: "Cascade ×2", cost: 70, tier: 2, cat: "buff_economy", color: "#f0e000" },
  { id: "shield", name: "Shield", cost: 80, tier: 2, cat: "defensive", color: "#4080ff" },
  { id: "time_warp", name: "Time Warp", cost: 50, tier: 2, cat: "buff_pieces", color: "#40ffcc" },
  { id: "reverse_controls", name: "Reverse", cost: 70, tier: 3, cat: "debuff_controls", color: "#ff2080" },
  { id: "mirage", name: "Mirage", cost: 75, tier: 3, cat: "debuff_visual", color: "#b040f0" },
  { id: "gold_digger", name: "Gold Digger", cost: 75, tier: 3, cat: "debuff_board", color: "#f0a020" },
  { id: "column_swap", name: "Column Swap", cost: 85, tier: 3, cat: "debuff_board", color: "#00f0f0" },
  { id: "purge", name: "Purge", cost: 60, tier: 3, cat: "defensive", color: "#ffffff" },
  { id: "fog_of_war", name: "Fog of War", cost: 90, tier: 3, cat: "debuff_visual", color: "#8080a0" },
  { id: "gravity_well", name: "Gravity Well", cost: 85, tier: 3, cat: "debuff_board", color: "#b040f0" },
  { id: "overcharge", name: "Overcharge", cost: 50, tier: 3, cat: "buff_economy", color: "#f0e000" },
  { id: "clone", name: "Clone", cost: 65, tier: 3, cat: "defensive", color: "#40ffcc" },
  { id: "rotation_lock", name: "Rotation Lock", cost: 85, tier: 4, cat: "debuff_controls", color: "#ff3c50" },
  { id: "blind_spot", name: "Blind Spot", cost: 100, tier: 4, cat: "debuff_visual", color: "#4040a0" },
  { id: "cross_firebomb", name: "Cross Fire", cost: 100, tier: 4, cat: "buff_board", color: "#ff6040" },
  { id: "weird_shapes", name: "Weird Shapes", cost: 100, tier: 4, cat: "debuff_controls", color: "#ff40a0" },
  { id: "narrow_escape", name: "Narrow Escape", cost: 100, tier: 4, cat: "buff_board", color: "#40ff90" },
  { id: "reflect", name: "Reflect", cost: 90, tier: 4, cat: "defensive", color: "#00f0f0" },
  { id: "magnet", name: "Magnet", cost: 90, tier: 4, cat: "buff_pieces", color: "#4080ff" },
  { id: "quicksand", name: "Quicksand", cost: 110, tier: 4, cat: "debuff_board", color: "#f0a020" },
  { id: "shapeshifter", name: "Shapeshifter", cost: 110, tier: 5, cat: "debuff_controls", color: "#b040f0" },
  { id: "shrink_ceiling", name: "Shrink Ceiling", cost: 110, tier: 5, cat: "debuff_geometry", color: "#ff3c50" },
  { id: "wide_load", name: "Wide Load", cost: 120, tier: 5, cat: "debuff_geometry", color: "#4080ff" },
  { id: "tilt", name: "Tilt", cost: 160, tier: 5, cat: "debuff_controls", color: "#ff2080" },
  { id: "flip_board", name: "Flip Board", cost: 180, tier: 5, cat: "debuff_board", color: "#f0a020" },
  { id: "death_cross", name: "Death Cross", cost: 250, tier: 5, cat: "debuff_board", color: "#ff3c50" },
];

const TIER_COLORS = { 1: "#ffffff", 2: "#40ff90", 3: "#4080ff", 4: "#b040f0", 5: "#ff3c50" };
const CAT_LABELS = {
  debuff_visual: "Visual", debuff_board: "Board", debuff_controls: "Control",
  debuff_geometry: "Geometry", buff_board: "Buff:Board", buff_pieces: "Buff:Piece",
  buff_economy: "Buff:Econ", defensive: "Defense",
};

export default function AllIcons() {
  const [viewMode, setViewMode] = useState("tier");

  const grouped = viewMode === "tier"
    ? [1,2,3,4,5].map(t => ({ label: `TIER ${t}${t===1?" · FREE":t===2?" · 100¢":t===3?" · 300¢":t===4?" · 600¢":" · 1200¢"}`, items: ALL_ABILITIES.filter(a => a.tier === t), color: TIER_COLORS[t] }))
    : Object.keys(CAT_LABELS).map(cat => ({ label: CAT_LABELS[cat], items: ALL_ABILITIES.filter(a => a.cat === cat), color: "#ffffff" }));

  return (
    <div style={{
      minHeight: "100vh",
      background: "radial-gradient(ellipse at 50% 10%, #0e0a2a, #06060f 60%)",
      padding: "20px 16px",
      ...
      <div style={{ textAlign: "center", marginBottom: 16 }}>
        <div style={{ fontSize: 13, color: "#ffffff", fontWeight: 700, letterSpacing: 5 }}>ALL 32 ABILITY ICONS</div>
        <div style={{ fontSize: 9, color: "#ffffff33", marginTop: 4, letterSpacing: 2 }}>Stackcraft 2 · Ability System</div>
        <div style={{ display: "flex", gap: 6, justifyContent: "center", marginTop: 12 }}>
          {["tier", "category"].map(m => (
            <button key={m} onClick={() => setViewMode(m)} style={{
              background: viewMode === m ? "rgba(255,255,255,0.06)" : "rgba(255,255,255,0.02)",
              border: `1px solid ${viewMode === m ? "#ffffff33" : "#ffffff0a"}`,
              borderRadius: 6, padding: "5px 14px", cursor: "pointer",
              color: viewMode === m ? "#fff" : "#ffffff55",
      ...
      {grouped.map(g => (
        <div key={g.label} style={{ marginBottom: 20, maxWidth: 800, margin: "0 auto 20px" }}>
          <div style={{
            fontSize: 9, fontWeight: 700, color: g.color, letterSpacing: 3,
            marginBottom: 8, opacity: 0.7,
          }}>{g.label}</div>
          <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
            {g.items.map(a => (
              <div key={a.id} style={{
                width: 96, padding: "10px 6px 8px",
                background: "rgba(255,255,255,0.02)",
                border: "1px solid rgba(255,255,255,0.05)",
                borderRadius: 8, textAlign: "center",
                display: "flex", flexDirection: "column", alignItems: "center", gap: 4,
              }}>
                <SkillIcon icon={a.id} color={a.color} size={36} glow />
                <div style={{ fontSize: 7, color: "#ffffffaa", letterSpacing: 0.5, lineHeight: 1.3 }}>{a.name}</div>
                <div style={{ fontSize: 7, color: a.color, opacity: 0.6, fontFamily: "monospace" }}>★{a.cost}</div>
              </div>
            ))}
          </div>
        </div>
